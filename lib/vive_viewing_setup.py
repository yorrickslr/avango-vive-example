...
class ViveViewingSetup:
    def __init__(
        ...
        if SCENEGRAPH is None:
            print("ERROR: scene graph instance missing")
            quit()
        ...
        # notice, that the vive screen transforms and translations are
        # automatically computed by the vive. Do not try to enter them
        ...
        #    EyeDistance ## distance between both eyes in meters
        ...
        # connect orientation/position data from vive sensor
    ...
    def run(self, LOCALS, GLOBALS):
        # GuaVE fails to start on windows with exception
